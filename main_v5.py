...
# 3. 规划师 (Planner) 节点 (M4.2 流式升级版)
async def planner_node(state: AgentState) -> dict:
    print("--- 正在调用 [规划师 (M4.2 流式)] ---")
    task = state.get("task")
    ...
    print("--- [规划师 M4.2] 正在流式生成计划... ---")
    
    # 1. (关键) 流式调用 LLM 来"思考"计划
    full_plan_str = ""
    async for chunk in llm.astream(messages):
        chunk_content = chunk.content or ""
        full_plan_str += chunk_content
        
        # (!! 关键 !!) "直播"规划师的"思考过程" (即, 正在生成的 JSON)
        yield {"plan": chunk_content} 
    
    # 2. 计划生成完毕, 解析它
    print(f"--- [规划师 M4.2] 计划生成完毕: {full_plan_str} ---")
    
    try:
        # 移除 LLM 可能添加的 markdown 标记
        if "```json" in full_plan_str:
             full_plan_str = full_plan_str.split("```json")[1].split("```")[0]
        
        plan_steps = json.loads(full_plan_str.strip())
    except Exception as e:
        print(f"--- [规划师 M4.2] 计划解析失败! {e} ---")
        plan_steps = ["Research", "Write"] # 降级, 确保流程能继续
    
    # 3. (关键) yield 最终的、解析后的 plan 列表
    # (注意: 我们不再流式 yield 这个, 我们只 yield 最终状态)
    yield {
        "plan": plan_steps, # (这是最终的、解析后的 plan 列表)
...
# 5. 写手 (Writer) 节点 (M4.1 流式升级版)
async def writer_node(state: AgentState) -> dict:
    """
    (M4.1) 这是一个"异步生成器"节点。
    它将流式地查询 RAG 并流式地返回 LLM 响应。
    """
    print("--- 正在调用 [写手 (M4.1 流式)] ---")
    task = state.get("task")
    
    # 1. M4.1 核心: "异步 RAG 查询" (Async Querying)
    print("--- [写手 M4.1] 正在从 pgvector 异步检索... ---")
    query_engine = index.as_query_engine()
    
    # (关键) 使用 .aquery() 而不是 .query()
    rag_context = await query_engine.aquery(task)
    
    print("--- [写手 M4.1] 记忆检索完毕 ---")
    
    # 2. 构建提示 (Prompt)
    prompt = f"""
    你是一个专业的科技报告写手。
    根据 [任务] 和你从"记忆"中检索到的 [RAG 上下文], 撰写一份专业的总结报告。
    ...
    print("--- [写手 M4.1] 正在流式生成报告... ---")
    
    # 3. M4.1 核心: "流式"调用 LLM 并 "yield" 块
    full_report = ""
    
    # (关键) 使用 .astream() 而不是 .invoke()
    async for chunk in llm.astream(messages):
        chunk_content = chunk.content or ""
        full_report += chunk_content
        
        # (!! 关键 !!)
        # 我们不是在"返回", 我们是在"屈服"(yield)
        # 这会把 token 块"直播"给 LangGraph 和 FastAPI
        yield {"final_report": chunk_content}

    # 4. 当循环结束时, yield 最终的完整状态
    print("--- [写手 M4.1] 报告生成完毕 ---")
    yield {"final_report": full_report, "current_step": "END"}
