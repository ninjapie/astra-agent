...
async def analyst_node(state: AgentState) -> dict:
    ...
    if previous_results:
        ...
        for i, res in enumerate(previous_results):
            found_urls = re.findall(r'https?://[^\s\)]+', res)
    ...
    prompt = f"""
      ...
      1. **信息不足？** -> `scrape_website` (不要重复抓取已读 URL)。
...
def analyst_router(state: AgentState) -> str:
    """
    通用行为路由：
    1. 没调工具? -> 思考结束 -> 去 Writer (汇报) 或 END (仅当已有图表且无需汇报时)
    2. 调了工具且出图了? -> 去质检
    3. 调了工具但没出图? -> 回环继续
    """
    ...
    # [关键修复] 
    # 如果没有生成任何视觉产物 (Image/HTML)，说明这是一个纯文本分析任务。
    # 必须强制去 Writer，否则 Analyst 的分析结果无法传达给用户 (前端不显示 Analyst 的思考)。
    if not (has_image or has_html):
        print("--- [路由] 纯文本任务 -> 强制 Writer 汇报 ---")
        return "Writer"
        
    # 如果有图，且计划里没 Write，才允许直接结束
    plan = state.get("plan", [])
