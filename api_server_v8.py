...
from main_v8 import app as astra_app # 导入 M2 中编译好的 LangGraph app
from dotenv import load_dotenv
from main_v8 import engine, vector_store
...
#         # 如果是 "continue" (批准)，输入是 None (表示继续下一步)
...
#         # 仍然使用 astream_events
#         # 2. 运行流
#         # (注意: 我们不需要特意区分 start/continue, LangGraph 会根据 inputs 是否为 None 自动处理暂停恢复)
...
#                 # print(f"DEBUG: Tool End Event: {event}") # 调试用
                
#                 # 确保是我们的 python_interpreter 工具
#                 if event["name"] == "python_interpreter":
#                     # 获取工具的返回值 (output)
#                     # 注意: LangGraph 的 output 结构有时会嵌套
...
#         # 3. [M9 关键] 流结束了, 但流程真的结束了吗?
#         # 检查当前的 State 快照
#         snapshot = astra_app.get_state(config)
#         if snapshot.next:
#             # 如果还有 "next" (比如 "Researcher", "Analyst")，说明是被 interrupt 了
