...
class AgentState(TypedDict):
    ...
    # [修复 2] 使用 Annotated 包装，解决 "Can receive only one value" 错误
...
async def analyst_node(state: AgentState) -> dict:
    ...
    return {
        ...
        # [修复 3] 删除 "visual_critique": "PASS"，避免与 VisualCritic 节点冲突
    }
...
async def visual_critic_node(state: AgentState) -> dict:
    ...
    critic_prompt = """
    你是一个苛刻的数据可视化专家。请检查这张图表是否存在以下问题：
    1. **文字重叠**: 标题、标签或图例是否重叠导致无法阅读？
    2. **乱码**: 是否出现方块口口等乱码？
    3. **配色丑陋**: 是否只有单一颜色（除非是单变量）？
    4. **布局问题**: 图片是否被截断？
    
    如果图表质量合格，请**仅回复** "PASS"。
    如果存在问题，请简要描述问题并给出 Python 代码修改建议。
    """
...
workflow.add_conditional_edges(
    ...
    {"Analyst": "Analyst", "VisualCritic": "VisualCritic", "Writer": "Writer"}
)
