...
def writer_node(state: AgentState) -> AgentState:
    ...
    # 1. M4 核心: "RAG 查询" (Querying)
    # 我们不再从 state.get("research_data") 读取
    # 我们从 LlamaIndex (pgvector) "查询"
    
    print("--- [写手 M4] 正在从 pgvector 检索相关记忆... ---")
    
    # 2. 创建查询引擎 (Query Engine)
    # 我们可以添加 "Filters" 来确保只检索与此 "task" 相关的数据
    # (为了 M4 简化, 我们先查询所有, 但筛选是最佳实践)
    query_engine = index.as_query_engine(
        # filters=MetadataFilters(
        #     filters=[ExactMatchFilter(key="task", value=task)]
        # )
    )
    
    # 3. 执行 RAG 查询
    rag_context = query_engine.query(task)
    ...
    return {
        "final_record": report, # (我们把 M2 的 'final_report' 错拼了, 修正一下)
        "current_step": "END"
    }
